...
SELECT 
    ...
    CAST(AVG(julianday(STRFTIME('%Y-%m-%d', order_delivered_customer_date)) - 
             julianday(STRFTIME('%Y-%m-%d', order_estimated_delivery_date))) AS INTEGER) AS Diferencia_Entrega
FROM olist_orders
WHERE 
